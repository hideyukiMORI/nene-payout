...
// 規約 04 I18N-20: 全ロケール shape 一致 CI ゲート。
//
// 0.2.0 の expectCatalogParity は **vitest の test() を自己登録しない**（実測: dist/parity.js は
// 違反時に throw するだけのアサーション）。トップレベルで呼んでもゲートは落ちるが、失敗が
// 名前のない収集時エラーになる。it() で包んで、何のゲートが落ちたかがレポートに出るようにする。
// 実 API は (catalogs, options) の2引数形（04 §I18N-20 の1引数例は 0.2.0 実装前の草稿）。
