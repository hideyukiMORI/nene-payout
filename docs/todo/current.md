...
- [ ] 🔒 **exemplar アンカー3本（I18N-6 / I18N-20 / I18N-22）＝植えない判断で W1 送り** — Issue #169（close 済・調査は同 Issue のコメント）
  - 3本とも**指し先が当の条文に未準拠**で、準拠する手段が存在しない。根本原因は
    `@hideyukimori/nene2-i18n` が未 publish（`private: true` / npm E404 / `exports` に `./testing` 不在）
  ...
  - ブロッカー: **nene2-i18n の public 化**（fleet-tooling 担当・施主判断待ち）
...
W1 の残りも nene2-i18n の publish 待ち（fleet-tooling 側）で、**payout 単独で動かせる玉は少ない**。
手番が来ているのは **PR #123（ウィジェット）のコンフリクト解消**と Dependabot 13本の処理。
フロントの管理 UI 横展開は一巡済み。

既知の未処理（コード外）:
- Issue #170: `NENE_PAYOUT_API_URL` の正本が無くポート変更に Vite proxy が追随しない — **施主判断で記録のみ・実装しない**
- `.claude/worktrees` 385M — #179 / PR #180 で ignore 済み。**worktree 実体の掃除は別途**
